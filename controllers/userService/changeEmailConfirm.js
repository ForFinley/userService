...
function validate(body, res) {
  if (!body.changeEmailHash) {
    res
      .status(400)
      .send(httpUtil.createResponse(400, 'ERROR : Missing changeEmailHash.'));
    return false;
  }
  if (!body.email) {
    res
      .status(400)
      .send(httpUtil.createResponse(400, 'ERROR : Missing email.'));
    return false;
  }
...
module.exports.handler = async function(req, res) {
  ...
  try {
    ...
    if (user.Count > 0) {
      return res
        .status(400)
        .send(httpUtil.createResponse(400, 'ERROR - email in use.'));
    } else {
